...
public class slideMoveScript : MonoBehaviour
{
    ...
    public int leftx, centerx, rightx, y;
    // Use this for initialization
    void Start () {
        leftx = 850 -300;//pc 850
        centerx = 925;//pc 625 
        rightx = 1475 + centerx;//pc 2100
        y = 525;//pc 400
    }
    private void Update()
    {
    ...
    //print("moveSlide " +moveSlide +" CurrentSlide "+ currentSlide);
        if(moveSlide!=currentSlide  && moveSlide != -1)
        {
            if (moveSlide > currentSlide)
            {
                    ...
                    Slides[currentSlide].transform.position = Vector3.MoveTowards(Slides[currentSlide].transform.position, new Vector3(-leftx, y, 0), SlideMoveSpeed * Time.deltaTime);
                    Slides[currentSlide + 1].transform.position = Vector3.MoveTowards(Slides[currentSlide + 1].transform.position, new Vector3(centerx, y, 0), SlideMoveSpeed * Time.deltaTime);


                print(Slides[currentSlide].transform.position.x);
                if (Slides[currentSlide].transform.position.x <= -(leftx-1))
                {
                    print("Entred next");
            ...
            if (moveSlide < currentSlide) {
                ...
                if (Slides[currentSlide - 1].transform.position.x <= rightx-1) {
                    Slides[currentSlide].transform.position = Vector3.MoveTowards(Slides[currentSlide].transform.position, new Vector3(rightx, y, 0), SlideMoveSpeed * Time.deltaTime);
                    Slides[currentSlide - 1].transform.position = Vector3.MoveTowards(Slides[currentSlide - 1].transform.position, new Vector3(centerx, y, 0), SlideMoveSpeed * Time.deltaTime);
                }

                print(Slides[currentSlide].transform.position.x);
                if (Slides[currentSlide].transform.position.x >= rightx-1)
                {
                    print("Entred prev");
                    currentSlide = currentSlide - 1;
                }
               ...
               // print("translate "+ Slides[currentSlide].transform.position);
                //Slides[currentSlide-1].transform.position = new Vector3(centerx, y, 0);
   ...
   //     print("Setting up slides...");
        ...
        for (int i=0;i<HeroIcons.Length;i++) {
         ...
         //   print("i  is " + i+ "currentSlide "+ currentSlide);
            ...
            {//set before
              ...
              //  print("Position Before parent "+ Slides[i].transform.position);
               ...
               // print("Position after parent " + Slides[i].transform.position);
                Slides[i].transform.position = new Vector3(-leftx, y, 0);
            ...
            else if (i == currentSlide)//set in frame
            {
                Slides[i] = Instantiate(SlidePrefab);
                Slides[i].transform.SetParent(SlideParent.transform);
                Slides[i].transform.position = new Vector3(centerx, y, 0);
                Slides[i].GetComponent<slideScript>().Hero = HeroIcons[i].GetComponent<IconInfoScript>().hero;
                Slides[i].GetComponent<slideScript>().initiateSlide();
            ...
            else//set after frame
            {
                Slides[i] = Instantiate(SlidePrefab);
                Slides[i].transform.SetParent(SlideParent.transform);
                Slides[i].transform.position = new Vector3(rightx, y, 0);
                Slides[i].GetComponent<slideScript>().Hero = HeroIcons[i].GetComponent<IconInfoScript>().hero;
                Slides[i].GetComponent<slideScript>().initiateSlide();
    ...
    public void nextSlide() {
        ...
        /*if (currentSlide + 1 < HeroIcons.Length)
        {
         //   print("Going To next hero");
            Slides[currentSlide].transform.position = new Vector3(rightx, y, 0);
            currentSlide++;
            Slides[currentSlide].transform.position = new Vector3(centerx, y, 0);
        }*/
    ...
    public void previousSlide()
    {
        ...
        /*if (currentSlide - 1 >= 0)
        {
            Slides[currentSlide].transform.position = new Vector3(-753, y, 0);
            currentSlide--;
            Slides[currentSlide].transform.position = new Vector3(centerx, y, 0);
        }*/
        if (moveSlide - 1 >= 0)
